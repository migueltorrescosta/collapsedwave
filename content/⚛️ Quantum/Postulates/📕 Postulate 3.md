...
tags:
  - "#quantum-theory"
---
If we measure a [[📘 Quantum state]] $\psi$ with result $\lambda \in \mathbb{R}$, Then immediately after the [[📘 Measurement]], the system will be in a state $\psi'$ satisfying $$\hat{f} \psi ' = \lambda \psi'$$
>[!info]-
> $\hat{f}$ is the [[📘 Self-Adjoint Operator]] associated with $f$. 

# Properties

If a [[quantum system]] is described by a [[📘 Quantum state]] $\psi \in H$, the probability distribution of the [[📘 Classical observable]] $f$ is given by $\mathbb{E}[f^m] = \braket{\psi, ( \hat{f} )^m \psi}$
